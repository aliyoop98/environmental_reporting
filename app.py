...
for uploaded in uploaded_files:
    ...
    df_clean['DateTime'] = pd.to_datetime(df_clean['Date'].dt.strftime('%Y-%m-%d ') + df_clean['Time'].astype(str), errors='coerce')
    dfs[name] = df_clean
    if has_fridge and has_freezer:
        ...
        rng = {'Temperature':(15,28) if 'olympus' in lname else (15,25),'Humidity':(0,60)}
    ranges[name] = rng
...
for name, df in dfs.items():
    ...
    df_sel = df[(df['Date'].dt.year==year)&(df['Date'].dt.month==month)].sort_values('DateTime').reset_index(drop=True)
    if df_sel.empty:
    ...
    for col in plot_keys:
        ...
        base = alt.Chart(df_plot).mark_line().encode(x='DateTime:T',y='Value:Q')
        mn,mx=rng[col]
        rules=[]
        if mn is not None:
            rules.append(alt.Chart(pd.DataFrame({'y':[mn]})).mark_rule(color='red',strokeDash=[4,4]).encode(y='y:Q'))
        if mx is not None:
            rules.append(alt.Chart(pd.DataFrame({'y':[mx]})).mark_rule(color='red',strokeDash=[4,4]).encode(y='y:Q'))
        points = alt.Chart(df_plot[df_plot['OutOfRange']]).mark_circle(color='red',size=50).encode(x='DateTime:T',y='Value:Q',tooltip=['DateTime','Value'])
        chart = base + points
        for r in rules:
            chart+=r
        st.altair_chart(chart.properties(title=f"{title} - {col}").interactive(),use_container_width=True)
